...
if ($user['login_status'] == 1) {
	...
	if ($user_fav_yet >= 1) {
		$game['fav_game'] = '<div id="favbutton"><a href="#">'.GAME_UNFAVOURITE.'</a></div>';
	}
	else {
		$game['fav_game'] = '<div id="favbutton"><a href="#">'.GAME_FAVOURITE.'</a></div>';
	}
}
else {
	$game['fav_game'] = '<a href="'.$user['login_link'].'">'.LOGIN.'</a>';
}

// Report game button
if ($setting['report_permissions'] == "1" || $setting['report_permissions'] == "2" && $user['login_status'] == 1) { 
	$game['report_game'] = '<div id="reportgame"><a href="#" onclick="ShowPopup(\'ava-popup\', \''.$setting['site_url'].'/includes/forms/game_report_form.php?id='.$row2['id'].'\', \''.GAME_REPORT.'\'); return false">'.GAME_REPORT.'</a></div>';
}
