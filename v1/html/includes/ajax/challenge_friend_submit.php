...
if ($user['password'] == $code && $user['banned'] == 0 && $are_friends == 1) {
	...
	$already_challenged = mysql_result(mysql_query("SELECT COUNT(*) as Num FROM ava_messages WHERE highscore_game_id = $highscore[game] AND user_id = $friend_id AND `read` = 0"), 0);
