...
function get_name($name) {
 ...
 /**
 * 去掉字符串中的(- _ \)等字符替换为空格,生成把字符串中每个单词的首字符转换为大写
 * @param String $name
 * @author Roble
 * @created 2013-12-5
 */
function get_title($name) {
    $name = preg_replace("/[\W]+/", ' ', $name);
    $name = preg_replace("/^_+|_+$/", '', ucwords($name));
    return $name;
}
/**
 * 获得分类路径
...
function get_url($params) {
    global $setting;
    return $setting['site_url'].'/'.$params;
}
 ...
 * @param String $type
 */
function error_msg($msg, $type=REGISTER) {
    if($type == REGISTER) {
        return  '<div class="error_ms">'.$msg.'</div>';
    }
}
/**
 * 发送邮件
 * @global string $_system['smtp']
 * @param string $to
 * @param string $from
 ...
 * @param string $header
 */
function send_mail($to, $from='', $subject='', $message='', $html_format=0, $header='') {
    $_system['smtp'] = array(
        'contact' => 'mh@noez.com',
        'host' => 'localhost',
        'port' => 25,
    ...
    if($_system['smtp']) {
        $from = empty($from) ? $_system['smtp']['from'] : $from;
        ini_set("SMTP", $_system['smtp']['host'] );
        ini_set('smtp_port', $_system['smtp']['port']);
...
function GenerateRating($rating, $location) {
	...
	if ($location == 'view_game') {
		...
		$empty_star = '<i class="star_empty"></i>';
                //$half_star = '<img src="'.$setting['site_url'].$setting['template_url'].'/images/'.$template['homepage_half_star'].'" alt="Rating star" />';
                $star = '<i class="star_full"></i>';
	}


	if ($rating <= 0  ){$rating_images = $empty_star.$empty_star.$empty_star.$empty_star.$empty_star;}
	//if ($rating >= 0.5){$rating_images = $half_star.$empty_star.$empty_star.$empty_star;}
	if ($rating >= 1  ){$rating_images = $star.$empty_star.$empty_star.$empty_star;}
	//if ($rating >= 1.5){$rating_images = $star.$half_star.$empty_star.$empty_star;}
	if ($rating >= 2  ){$rating_images = $star.$star.$empty_star.$empty_star;}
	//if ($rating >= 2.5){$rating_images = $star.$star.$half_star.$empty_star;}
	if ($rating >= 3  ){$rating_images = $star.$star.$star.$empty_star;}
	//if ($rating >= 3.5){$rating_images = $star.$star.$star.$half_star;}
	if ($rating >= 4  ){$rating_images = $star.$star.$star.$star;}
...
function FormatDate($str, $depth) {
    ...
    } elseif($depth == 'admin_date') {
        if($str != '0000-00-00 00:00:00') {
            $s = date($setting['date_format'],strtotime($str));
        } else {
            $s = 'No record';
        }
    } elseif($depth == 'admin_datetime') {
        if($str != '0000-00-00 00:00:00') {
            $s = date($setting['date_format'].', H:i',strtotime($str));
        } else {
            $s = 'No record';
...
function GameData($raw_data, $type) {
	...
	$game = array('hits' => $raw_data['hits'], 'id' => $raw_data['id'], 'instructions' => nl2br($raw_data['instructions']), 'plays' => $raw_data['hits'],  'highscores' => $raw_data['highscores'],  'seo_url' => $raw_data['seo_url'],  'category' => $raw_data['category_id'], 'submitter' => $raw_data['submitter']);
...
function SendPM($subject, $message, $to_user_id, $highscore_game_id = 0) {
	global $setting, $user;

	$date = date("Y-m-d H:i:s");

	mysql_query("INSERT INTO ava_messages (user_id, sender_id, sender_name, title, message, date, ip, highscore_game_id)
				VALUES ('$to_user_id', '$user[id]', '$user[username]', '$subject', '$message', '$date', '$_SERVER[REMOTE_ADDR]', $highscore_game_id)") or die (mysql_error());

	// Update user messages counter
	mysql_query("UPDATE ava_users SET messages = messages + 1, new_pms = 1 WHERE id = $to_user_id") or die (mysql_error());
}
...
//转换采集产生的CDN字符
function CDNUrlConvert($url) {
    global $setting;
    if(strpos($url, '{SITE_CDN}') !== false) {
        $url = str_replace('{SITE_CDN}', $setting['store_url'], $url);
    }
    return $url;
}
 ...
 */
function landscape($width, $height, $new_width) {
        $width = abs(intval($width + 0));
        $height = abs(intval($height + 0));
        $new_width = abs(intval($new_width + 0));
        if ($width < 1 || $height < 1 || $new_width < 1)return array(0, 0);
        $ratio = $height / $width;
 ...
 * @created 2013-5-13
 */
function set_history($name, $count=5){
    if(!$name) {
        return false;
    }
    $cookieName='history';
    ...
    if(!$_COOKIE['history']) {
        setcookie($cookieName, q($name), time() + 86400, '/');
    } else {
        $cookie = $_COOKIE['history'];
        $cookie = explode(',', $cookie);
        array_unshift($cookie, q($name));
        $cookie = array_unique($cookie);
        if(count($cookie) >= $count) {
            array_pop($cookie);
        }
        $cookie_str = implode(',', $cookie);
        setcookie($cookieName, $cookie_str, time() + 86400, '/');
    }
}
